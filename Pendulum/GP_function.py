...
class GP:
    ...
    def f_std(self, X):
        v1 = self.gp1.predict([X], return_std=True)[1][0]
        v2 = self.gp2.predict([X], return_std=True)[1][0]
        self.v1 = v1
        self.v2 = v2

    def z_std(self, x, y):
        # y_pred_std = [self.f_std(np.array([x[i, j], y[i, j]]))
        #               for i in range(100) for j in range(100)]
        # return (np.array(y_pred_std)[:, 0]+np.array(y_pred_std)[:, 1]).reshape((100, 100))
        return np.array([self.f_std(np.array([x[i, j], y[i, j]]))
                         for i in range(100) for j in range(100)]).reshape((100, 100))
